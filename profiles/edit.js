...
import EditProfileCss from './edit.css';
...
class AddProfileAppModal extends React.Component {
	...
	componentDidMount(){
		...
		trendFun.jsGetRequest(app_list_url, function(response){
			...
			$.each(response.results, function(i,e){
				if ($.inArray(e.id, ids) == -1 && e.type!=3) {
						...
						html+='	  <div id="app_'+e.id+'" class="appviewall hmdm-app-summary">';
		...
		//$(document).on("click", ".appviewall input[type=checkbox][class=ember-checkbox]", function(){
			 //$(this).parent().click();
			 //return;
		...
		$(".selectall").click(function(e) {
		   ...
		   $(".appviewall input[type=checkbox][class=ember-checkbox]").each(function(index, element) {
		...
		$(".selectnone").click(function(e) {
		   ...
		   $(".appviewall input[type=checkbox][class=ember-checkbox]").each(function(index, element) {
			...
			$("#selectnumsall").text('0');
		});
		
		
		//search
		$("#searchprofile").click(function(e) {
			var searchprofile=$.trim($("#profilename").val());
			/*
			if(searchprofile==""){
			...
			$("#listappall div[class=ember-view]").each(function(i, e) {
		...
		$("#profilename").keyup(function(e){  
			if(e.keyCode==13 || e.which==13){
			   $("#searchprofile").click();
			   $("#profilename").blur();
			   setTimeout(function(){$("#profilename").focus();},200);
			}
		});
	}
	
	componentWillUnmount(){
		$(document).off('click', ".appviewall");
		$(document).off('mouseover', ".appviewall");
		$(document).off('mouseout', ".appviewall");
		$(document).off('click', ".appviewall input[type=checkbox][class=ember-checkbox]");
		$(document).off('click', ".selectall");
		$(document).off('click', ".selectnone");
	}
	...
	saveModalAddApp(){
		var ids=[];
		$("#listappall input[type=checkbox][class=ember-checkbox]:checked").each(function(index, element) {
					...
					<Modal.Dialog dialogClassName="modal-addbluetooth">
						...
						<Modal.Body>
							<div id="modalAddProfileApp">
								<h4 className="title">
									<input name="profilename" className="form-control" type="text" id="profilename" style={{"width":"604px"}} maxLength="40"  placeholder="Type an application name to search."  />
									<Button id="searchprofile">Search</Button>
								</h4>
								<div className="title titlefix">Select:
									<a href="javascript:;" className="selectall">All</a> | <a href="javascript:;" className="selectnone">None</a> (Selected: <span id="selectnumsall">0</span>)
								</div>
...
export default class EditProfile extends React.Component {
  ...
  componentDidMount(){
		...
		var profile_id = parseInt(this.props.params.id,10);
		trendFun.jsGetRequest('policy/template/'+profile_id+'/', function(response){
			 CURRENT_TEMPLATE = response;
		     var template=response;
			 var app_id=response.app_id; //single app
			 var single_app_flag=response.single_app
			 if(!single_app_flag){
			 	app_id=0;
			 }
			 
			 PUPLIC_APP.package=response.package_name;
			 PUPLIC_APP.app_id=response.app_id;
			 PUPLIC_APP.single_app=response.single_app;
			 
			 CURRENT_APPS = response.apps.slice(0);
			 $("#profilename").text(response.name);
			 $("#descripton").val(response.detail);
			 $("#watermark_enable").prop("checked",response.watermark_enable);
			$("#watermark_text").prop("disabled",!response.watermark_enable);
			...
			if($("#bluetooth_enable").prop("checked")==true){
			 ...
			 $("#watermark_text").val(response.watermark_text);
			 ...
			 if(PROFILE_TYPE==2){
				 CURRENT_APPS = response.local_apps.slice(0);
				$(".sandbox_div").show();
				$(".vmi_div").hide();
				$("#encrypt_file").prop("checked",response.sandbox_setting.encrypt_file);
				$("#restrict_cut").prop("checked",response.sandbox_setting.restrict_cut);
				$("#restrict_share").prop("checked",response.sandbox_setting.restrict_share);
				$("#restrict_capture").prop("checked",response.sandbox_setting.restrict_capture);
				$("#enforce_encryption").prop("checked",response.sandbox_setting.enforce_encryption);
				$("#root_check").prop("checked",response.sandbox_setting.root_check);
				$("#offline_period").val(response.sandbox_setting.offline_period);
				$("#note").hide();
			 }else{
				$(".sandbox_div").hide();
				$("#note").show();
		...
		$(document).on("click", ".appview input[type=checkbox][class=ember-checkbox]", function(event){
			 //$(this).parent().click();
			 //return;
			 if($(this).prop("checked")){
				$(this).parent().addClass("is-selected");
		...
		//$("#button_remove").click(function(e) {
			 ////$(".appview input[type=checkbox][class=ember-checkbox]:checked")
  	...
  	componentWillUnmount(){
		$(document).off('click', ".appview");
		$(document).off('mouseover', ".appview");
		$(document).off('mouseout', ".appview");
		$(document).off('click', ".appview input[type=checkbox][class=ember-checkbox]");
		$(document).off('click', ".selectall");
		$(document).off('click', ".selectnone");
  ...
  saveButton(){
		...
		var profile_id = parseInt(this.props.params.id,10);
		trendFun.jsPutRequest('policy/template/'+profile_id+'/', request, function(response){
  ...
  cancelButton(){
	...
	showApps(total_apps,current_apps,app_id){
			...
			$.each(total_apps, function(i,e){
				if(e.type != 3){
					...
					if($("#appview_"+e.id).length<=0){
						...
						html+='	  <div id="app_'+e.id+'" class="appview hmdm-app-summary '+isSelected+'">';
						html+='		  <img class="icon" src="'+ e.icon_url +'">';
						html+='		  <div class="name">'+e.name+'</div>';
		...
		parseID1(bluetoothRuleList, startId){
			var rowState = [];
			for(var i=0; i<bluetoothRuleList.length; i++)
				rowState[i] = false;
			this.setState({bluetoothRulelist: bluetoothRuleList, ruleStartid: startId, bluetoothRuleCheckboxStatus:rowState, checkBluetoothRuleAll:false});
		} 
	...
	return (
	  <div id="create_profile">
                ...
                <li>
                  ...
                  <div>
                      <div className="previewdiv dropdown">
							...
							<img src="../images/noicon.png" id="preview" width="140" height="150" className="btn-link dropdown-toggle" data-toggle="dropdown" href="#"/>
          ...
          <div className="panel fontSetting" id="applicationssetting">
            ...
            <div className="content">
              <ul className="detailLists">
                ...
                <li>
                  <dt style={{"width":"auto","fontWeight":"normal"}}>Select: <a href="javascript:;" className="selectall">All</a> | <a href="javascript:;" className="selectnone">None</a> (Selected: <span id="selectnums">0</span>)</dt>
                </li>
                <li>
			...
			{//<div className="hidden-lg content">
				//<li style={{"listStyle":"none"}}>Edit applications on PC</li>
			//</div>
			}
          </div>
		<div className="panel footerSetting" id="mdmsetting">
          ...
          <ul className="buttonGrp" style={{"marginLeft":"25px"}}>
